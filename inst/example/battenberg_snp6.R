...
ascat.bc = ascat.loadData(paste(TUMOURNAME, "_mutantLogR.tab", sep=""),
                          paste(TUMOURNAME, "_mutantBAF.tab", sep=""),
                          paste(TUMOURNAME, "_germlineLogR.tab", sep=""),
                          paste(TUMOURNAME, "_germlineBAF.tab", sep=""))
ascat.plotRawData(ascat.bc, parentDir=RUN_DIR)
...
foreach(chrom=1:length(chrom_names), .export=c("generate.impute.input.snp6","run.impute","combine.impute.output","GetChromosomeBAFs_SNP6","plot.haplotype.data")) %dopar% {
  ...
  cmd = paste("rm ", paste(TUMOURNAME, "_impute_input_chr", sep=""), chrom, "*K.txt*", sep="")
  system(cmd)
}
