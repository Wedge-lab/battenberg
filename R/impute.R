...
#' @export
run_haplotyping = function(chrom,
                           tumourname,
                           normalname,
                           ismale,
                           imputeinfofile,
                           ...
                           beagleoverlap=4)
{
  ...
  if (file.exists(paste(tumourname, "_alleleFrequencies_chr", chrom, ".txt", sep=""))) {
    ...
    if(usebeagle){
        ...
        imputeinputfile <- paste(tumourname,
                                 "_impute_input_chr",
                                 chrom, ".txt", sep="")
        vcfbeagle <- Battenberg:::convert.impute.input.to.beagle.input(imputeinput=imputeinputfile,
                                                          chrom=chrom)
        ...
        Battenberg:::writevcf.beagle(vcfbeagle,
                                     filepath=vcfbeagle_path)
        ## Run beagle5 on the files
        Battenberg:::run.beagle5(beaglejar=beaglejar,
        ...
        Battenberg:::writebeagle.as.impute(vcf=vcfout,
                                           outfile=outfile)
    }
    else{
        ...
        combine.impute.output(inputfile.prefix=paste(tumourname, "_impute_output_chr", chrom, ".txt", sep=""),
                              outputfile=paste(tumourname, "_impute_output_chr",
                                               chrom, "_allHaplotypeInfo.txt", sep=""),
                              is.male=ismale,
                              imputeinfofile=imputeinfofile,
                              region.size=5000000,
  ...
  if (file.exists(paste(tumourname, "_alleleFrequencies_chr", chrom, ".txt", sep=""))) {
    ...
    GetChromosomeBAFs(chrom=chrom,
                      ...
                      haplotypeFile=paste(tumourname, "_impute_output_chr",
                                          chrom, "_allHaplotypeInfo.txt", sep=""),
                      samplename=tumourname,
                      outfile=paste(tumourname, "_chr",
                                    chrom, "_heterozygousMutBAFs_haplotyped.txt", sep=""),
                      chr_names=chrom_names,
    ...
    GetChromosomeBAFs_SNP6(chrom=chrom,
                           alleleFreqFile=paste(tumourname, "_impute_input_chr",
                                                chrom, "_withAlleleFreq.csv", sep=""),
                           haplotypeFile=paste(tumourname, "_impute_output_chr",
                                               chrom, "_allHaplotypeInfo.txt", sep=""),
                           samplename=tumourname,
                           outputfile=paste(tumourname, "_chr",
                                            chrom, "_heterozygousMutBAFs_haplotyped.txt", sep=""),
                           chr_names=chrom_names)
  }

  # Plot what we have until this point
    plot.haplotype.data(haplotyped.baf.file=paste(tumourname, "_chr",
                                                  chrom, "_heterozygousMutBAFs_haplotyped.txt", sep=""),
                        imageFileName=paste(tumourname,"_chr",
                                            chrom,"_heterozygousData.png",sep=""),
                        samplename=tumourname,
                        chrom=chrom,
                        chr_names=chrom_names)
