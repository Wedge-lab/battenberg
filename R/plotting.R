...
create.bb.plot.subclones = function(bafsegmented, subclones, ploidy, rho, goodnessOfFit, pos_min, pos_max, subcl_min, subcl_max, is_subclonal, is_subclonal_maj, is_subclonal_min, chr.segs) {
  ...
  plot(c(1,nrow(bafsegmented)), c(0,5), type = "n", xaxt = "n", main = maintitle, xlab = "", ylab = "")
  abline(v=0,lty=1,col="lightgrey")
  segments(x0=pos_min, y0=subclones$nMin1_A, x1=pos_max, y1=subclones$nMin1_A, col="navy", pch="|", lwd=ifelse(is_subclonal_min, 6*subclones$frac1_A, 6), lend=1)
  ...
  segments(x0=subcl_min, y0=subclones$nMin2_A[is_subclonal], x1=subcl_max, y1=subclones$nMin2_A[is_subclonal], col="navy", pch="|", lwd=ifelse(is_subclonal_min[is_subclonal], 6*subclones$frac2_A[is_subclonal], 0), lend=1)
