germline_reconstruct_normal <-function(GERMLINENAME,NORMALNAME,GAMMA,KMIN,CENTROMERE_DIST,MIN_HET_DIST,GAMMA_LOGR,LENGTH_ADJACENT){
 ...
 #filter regions for those next to the centromere and 'short' (<200k)
  ...
  #STEP 2.1: identify LOH by inter-HET SNP regions # differentiating from HOM stretch in sample with logR < -0.8 #glcode 
  ...
  if (!is.null(non_LOH)){ # if regions of non_LOH exist after IVD-PCF, run window-based search 
    ...
    if (is.na(match(i,c(13,14,15,21,22)))){
      ...
      if (parm[nrow(parm),1]<(parm[nrow(parm),2]-CENTROMERE_DIST)){ #glcode
    ...
    if (!is.null(nrow(LOHall))){
      ...
      for (j in 1:nrow(LOHall)){
        ...
        if (nrow(m)>=hetSNP_number){
          ...
          print("less rows in LOH region than Het SNP number - turning all into Heterozygotes") # technically shouldn't happen in new version
          for (k in 1:nrow(m)){
            m$depth[k]=max(m$depth[k],10)
            m[cbind(k,2+m$a0[k])]=ifelse(m$depth[k]%%2==0,m$depth[k]/2,ceiling(m$depth[k]/2))
            m[cbind(k,2+m$a1[k])]=ifelse(m$depth[k]%%2==0,m$depth[k]/2,floor(m$depth[k]/2))
