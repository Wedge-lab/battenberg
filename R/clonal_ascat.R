...
#   library(RColorBrewer)
  ...
  if(nropt>0) {
	...
	# replaced plotting-extracted versions for the new ASCAT plotting functions (+ removed aberration reliability plot)
	...
	ascat.plotAscatProfile(n1all = nB, n2all = nA, heteroprobes = TRUE, ploidy = ploidy_opt1, rho = rho_opt1, goodnessOfFit = goodnessOfFit_opt1, nonaberrant = FALSE, ch = ch, lrr = lrr, bafsegmented = bafsegmented)
	...
	ascat.plotNonRounded(ploidy = ploidy_opt1, rho = rho_opt1, goodnessOfFit = goodnessOfFit_opt1, nonaberrant = FALSE, nAfull = nBfull, nBfull = nAfull, bafsegmented = bafsegmented, ch = ch, lrr = lrr)
...
run_clonal_ASCAT = function(lrr, baf, lrrsegmented, bafsegmented, chromosomes, segBAF.table, input_optimum_pair, dist_choice, distancepng = NA, copynumberprofilespng = NA, nonroundedprofilepng = NA, gamma_param, read_depth, uninformative_BAF_threshold, allow100percent, reliabilityFile=NA) # kjd 10-1-2014
{
  ...
  if(nropt>0) {
	...
	ascat.plotAscatProfile(n1all = nB, n2all = nA, heteroprobes = TRUE, ploidy = ploidy, rho = rho, goodnessOfFit = goodnessOfFit, nonaberrant = FALSE, ch = ch, lrr = lrr, bafsegmented = bafsegmented)
	if (!is.na(copynumberprofilespng)) { dev.off() }
	...
	ascat.plotNonRounded(ploidy = ploidy, rho = rho, goodnessOfFit = goodnessOfFit, nonaberrant = FALSE, nAfull = nBfull, nBfull = nAfull, bafsegmented = bafsegmented, ch = ch, lrr = lrr)
	if (!is.na(nonroundedprofilepng)) { dev.off() }
